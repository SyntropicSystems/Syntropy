...
- `AGENTS.md` — repo entry point (humans + AIs)
...
1. Identify which agent owns the relevant domain (check `.syntropy/system-of-work/ROUTER.md`)
...
Every domain maintains a **Domain State** section in its `CONTEXT.md` — a living snapshot of what is currently true in the domain. This is distinct from the load list (which is about *files to load*); domain state captures *current reality*.
