...
Every agent is composed of the same **9 Internal Components** — Capabilities, Attributes, Skills, Memory, Internal Context, Internal State, Traits, Policies, and Workflows — described by their job and purpose, never by implementation. A "Memory" is the history an actor draws on, not a database technology. A "Skill" is a task an actor can perform, not an API endpoint. This vocabulary is shared across every discipline — product, design, engineering, QA — so that anyone can reason about why an actor behaved the way it did.
